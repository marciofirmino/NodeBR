...
class Database {
  ...
  async remover(id) {
    ...
    const dados = await this.obterDadosArquivo();
    const indice = dados.findIndex(item => item.id == parseInt(id)); //passar funcao que retorno item
    if (indice == -1) {
  ...
  async atualizar(id, modificacoes) {
    ...
    await this.escreverArquivo([...dados, objetoAtualizar]);
    return true;
  }
}
